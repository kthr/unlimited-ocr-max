...
``tools/quantize_int8.py`` ships publicly and must run on a bare
``pip install numpy``, and carries its own copy of the widening today precisely
because it could not import this one. Getting bit patterns *into* numpy needs
a :class:`~max.driver.Buffer`, the one representation in reach that can name
bfloat16; that crossing lives in :mod:`unlimited_ocr_max.buffers`.
"""
